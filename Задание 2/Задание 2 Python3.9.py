# Python 3.9 Вариант кольцевого буфера в котором при заполнении буфера новые данные перезаписывают старые.
...
class RingBuffer(list):
    ...
    def dequeue(self): # берём из очереди если есть данные
        if self.buffer.count(float('inf')) == self.buffer_size:
            return print('Error: Ring buffer empty')
        else:
    ...
    def counter(self): # количество элементов в буфере в формате дроби, например 2/7
        ...
        return print(str(counter) + '/' + str(self.buffer_size))
...
# Python 3.9 Второй вариант кольцевого буфера в который после заполнения новые данные не попадают и выводится сообшене о заполнении.
...
class RingBuffer2(list):
    ...
    def enqueue(self, n): # ставим в очередь
        if float('inf') not in self.buffer:
            return print('Error: Ring buffer full')
        else:
